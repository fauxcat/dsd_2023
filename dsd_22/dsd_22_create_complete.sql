...
SELECT role_name,
       ...
       staff_lname
FROM ROLE
JOIN staff ON ROLE = role_id;

 -- Q2 proper query using WHERE
...
JOIN manifest ON manifest.cust_ord_id = cust_order.cust_ord_id
...
ORDER BY cat_name,
 ...
 -- Q6 - include the prod_id in the output
...
select cat_name, prod_name, cust_order.cust_ord_id, cust_fname, cust_lname, email from customer
join cust_order on cust_order.cust_id = customer.cust_id
join manifest on manifest.cust_ord_id = cust_order.cust_ord_id
join product on manifest.prod_id = product.prod_id 
join category on product.prod_cat = category.cat_id 
where prod_name like '%able%' ORDER BY cat_name ,prod_name;
